...
2026-09-12 范围更新：当前源码 0.8.0，DS01 工程已实现。用户确认重点融合生成段，不加入参考片研究，见 [生成段融合计划](generation-segments-plan.md)。后续 DS02–DS04／DS06 按 GS 切片细化；本文保留分镜方法、阶段 Skill 与质量验收要求，生成段的对象归属与执行顺序见 GS 计划，完成状态以 [TODO](TODO.md) 为准。

初始评估日期：2026-09-12。当时仅获评估与制定计划的授权；用户随后要求执行，DS01 工程已完成。后续版本沿用本地适配规则，未安装整套上游 Skill；真实分镜质量与生成效果仍待验。不能把初始规划状态当作当前进度。
...
| 用户所处位置 | 动作 | 应看到的结果 |
| --- | --- | --- |
| 分镜工作区 | 选择剧本、范围和制作方向，点击“设计分镜” | 可编辑镜头建议；原文依据、拍摄理由、景别机位、起始画面、结束状态 |
| 分镜建议 | 检查、修改、保留或忽略后应用 | 对应镜头卡与剧本/资产关系，并确认全片顺序；原有卡片位置和视频采用保留 |
| 分镜工作区 | 查看本镜“关键帧与参考图” | 静态关键帧提示词和所需图片计划；用于哪个段输入在生成段中绑定 |
| 资产库 | 需要时点击“准备图片提示词” | 该资产的图片文案与参考用途；不自动生成图片 |
| 生成段工作区 | 检查编排、参考图，准备视频提示词 | 本段完整文案、图像顺序、目标模式、生成时长和段内时间窗；可声明外部挂图缺口 |
| 生成段工作区 | 准备一次尝试、复制/导出制作包，之后导回视频 | 视频归入尝试成为候选，由用户按镜头选择实际区间；保留原有直接导入镜头入口 |
| 采用视频 | 选择“用于续接”或“独立机位” | 需要时截取真实画面并指向目标段输入；不强制每个视频都截帧 |

主界面沿用 0.7.0 的独立工作区，画布是关系视图，长表单不再挤入剧本卡。只在相应功能已经实现时显示可执行入口。每个镜头的默认阅读顺序是“拍什么 → 为什么这样拍 → 用什么图 → 去哪个生成段制作 → 还缺什么”，不让用户填写内部 ID、Markdown 标题或 JSON。
...
- 镜头存成片计划时长，生成段存目标生成时长及段内计划时间窗；GS03 按视频登记实际采用入出点。不同时间坐标不互相冒充，具体约束见 GS 计划；对白估时属于规划，生成后还要检查完整性。
...
| 起止状态、声音、时间 | 目前无对应结构字段 | 镜头笔记存起点/动作/终点、声音意图和计划剪辑时长；目标生成时长及段内时间窗只存生成段，不在镜头重复维护 |
| 全片顺序与生成段 | 只有场次 `shotOrder`，无生成段 | GS01 新增项目顺序与生成段编排；主段唯一覆盖、补拍和方案替代关系见 GS 计划 |
...
| 输入图与挂图计划 | `media`、`links` 与 `purpose/confirmed/fingerprint` | 生成段保存有顺序的实际输入槽位，指向媒体/资产或明确的待准备计划；镜头与资产保留创作需要和已有绑定，不能另存一份执行槽位 |
| MOTION 视频提示词 | 通用 `prompt` 无边界版本 | DS04 / GS02 在生成段增加独立 `videoPrompt`，绑定覆盖镜头、参考顺序、生成目标与输入哈希；旧镜头 `prompt` 作为来源保留 |
| 生成尝试 | 尚无对应记录 | GS02 在首次交付包前冻结完整计划输入，另记可补录的实际执行信息；格式版本与内容修订、输入哈希分开 |
| 视频采用与结果 | 镜头 `media.decision` | 结果归尝试，采用仍归镜头；GS03 增加实际区间和来源关联。旧采用未标区间需补标，不伪造生成尝试 |
| 上一段实际尾帧 | `frame` 与起始帧连线，尚无来源时间 | DS06/R4 增加源媒体、实际时间、采用区间依据与目标段输入；保留旧关系 |
...
每批单独交付和验收。DS 保留能力分类，实施按 GS01–GS04 推进；首次制作包必须包含尝试快照，GS03 才完成按实际区间选片，GS04 完成截帧续接。DS05 是资产方法增强，不要求四项能力一次全部上线。
...
| DS02 / GS01 镜头卡与编排 | 分镜确认、映射入卡、作品方向与阶段方法选择/自定义、项目全片顺序、生成段手动编排、取消/错误/重开恢复 | 中文界面完成剧本到镜头卡；重试不重复，过期不覆盖，旧候选/采用/布局保持；主段、补拍与历史方案不争夺覆盖 |
| DS03 / GS02 图片与参考准备 | 适配图片提示词 Skill；资产图按需准备；本镜关键帧文案、段参考槽位和外部挂图计划 | 已有图可以直接用；缺图准确定位；区分资产参考、首帧与全参考模式；没有图片理解仍可由用户完成正确绑定 |
| DS04 / GS02–GS03 段制作包与结果 | 视频提示词 Skill、经核对的 H3 格式、段内时间窗、制作包和首次尝试快照；随后导入结果并按镜头选片 | 文案与挂图一致；外部改写可追溯；剪辑时长、生成时长、段内窗口与实际区间分开；重复复制不制造尝试；补拍保留原段其他可用镜头 |
...
- DS05–DS06：按资产状态/选帧切片扩展对应模型和保存层，并补具体格式文档。复用已实现的项目库，不预建完整跨集状态机或通用工作流引擎。
