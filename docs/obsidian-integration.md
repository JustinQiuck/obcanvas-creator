...
状态：P0-A 已实现并在隔离 Obsidian 1.13.7 实例中通过验收，见 [验收记录](validation/p0-a.md)。原画布文件仍未引入，P0-B／P0-C 待实施。
...
以下路径均相对于本机复用来源 `/Users/aliceboy/项目文件/画布`，不是本仓库已存在的代码。该工作区有未提交修改，引入前必须记录选中文件的实际内容、来源版本及校验值，保留其 MIT 许可证和署名。
...
P0 建议在隔离测试资料库中采用以下结构，全部由插件创建，用户不需要手写字段。路径与字段在实际编码时固化，当前示意不代表文件已生成。
...
- 镜头笔记是内容的正式来源；布局 JSON 不再复制镜头正文或采用决定。P0 不实现多版本采用流程。
- 优先通过 Vault API 操作可见项目文件。更改现有正文时基于当前内容做受控更新；属性更新使用宿主提供的 frontmatter 接口，不自己编写 YAML 解析器。
...
P0-A 已完成，P0-B 是下一块可实施的代码范围。P0 全部通过后，进入 [总计划](plan.md) 中 P1 的三个镜头、多候选和采用状态验收。不能将显示一个空画布称为已完成打通。
...
- 待验证：原画布组件复用后的主题及输入边界、布局恢复、视频解码与外置素材访问。
- 当前交付包含 P0-A 插件与隔离测试环境；原画布业务代码和私人资料库配置未修改。
