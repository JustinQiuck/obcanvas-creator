...
import { isProductionAsset, assetStatus, projectRecords } from '../model';

export function Workbench({ records, editor, layout, viewports, media, openNote, extractions, storyboards, projectId, openSection }: { skills: VaultSkills; records: VaultRecords; editor: EditorSession; layout: VaultLayout; viewports: Viewports; media: VaultMedia; openNote: (path: string) => Promise<void>; extractions: ExtractionService; storyboards: StoryboardService; owner: string; projectId: string; openSection: (section: 'assets' | 'storyboard', scriptId: string) => void }) {
  const extractionState = useSyncExternalStore(extractions.subscribe, extractions.getSnapshot);
  const storyboardState = useSyncExternalStore(storyboards.subscribe, storyboards.getSnapshot);
  const fullCatalog = useSyncExternalStore(records.subscribe, records.getSnapshot);
  const catalog = { ...fullCatalog, records: projectRecords(fullCatalog.records, projectId) };
  ...
  const project = fullCatalog.records.find(r => r.kind === 'project' && r.id === projectId);
  const shots = orderedShots(scene, fullCatalog.records);
  ...
  async function run(action: () => Promise<void> | void, save = true) {
    ...
    try {
      ...
      if (save) { await extractions.flushDrafts(); await storyboards.flushDrafts(); }
      await action();
  ...
  async function ensureScene() {
    ...
    const created = await records.create('scene', undefined, projectId); setSceneId(created.id); return created.id;
  }
  ...
  async function create(kind: RecordKind) {
    await run(async () => {
      ...
      const r = await records.create(kind, id, projectId);
  ...
  function requestDelete(target = selected) {
    if (!target || extractions.getSnapshot().busy || storyboards.getSnapshot().busy) return;
    void run(async () => {
  ...
  function confirmDelete() {
    if (!deleting) return;
    void run(async () => {
      if (extractions.getSnapshot().busy || storyboards.getSnapshot().busy) throw new Error('请等待当前 AI 分析完成或取消后再删除。');
  ...
  return <section className="obcanvas-workbench obcanvas-free" aria-label="影视画布工作台" onKeyDown={e => {
    ...
    </select><button disabled={busy} onClick={() => void create('scene')}>新建场次</button><button disabled={busy || !scene} onClick={() => void run(() => { editor.select(scene!.id); setSelectedKey(`r:${scene!.id}`); setPanel('detail'); })}>场次名称</button><span className="obcanvas-hint">当前剧本项目的场次画布</span></header>
    ...
    <div className="obcanvas-free-stage" ref={board} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); const rect = e.currentTarget.getBoundingClientRect(); void importFiles(Array.from(e.dataTransfer.files), { x: (e.clientX - rect.left - viewport.x) / viewport.k, y: (e.clientY - rect.top - viewport.y) / viewport.k }); }}>
      ...
      {panel && <aside className="obcanvas-inspector obcanvas-shot-card" aria-label={panel === 'detail' ? '卡片详情' : panel === 'tasks' ? '制作待办' : '镜头顺序'}>
        <div className="obcanvas-card-heading"><h2>{panel === 'detail' ? selected?.label ?? '场次' : panel === 'tasks' ? '下一步做什么' : '镜头顺序'}</h2><span>{panel === 'detail' && selected && <button disabled={busy || state.saving || extractionState.busy || storyboardState.busy} onClick={() => requestDelete()}>{selected.record ? '删除卡片' : '移除素材卡'}</button>}<button aria-label="关闭详情" onClick={() => void run(() => setPanel(''))}>×</button></span></div>
        {panel === 'detail' && <>
          {selected?.record?.kind === 'script' && <>
            <p className="obcanvas-hint">人物、场景和道具在项目资产库统一管理。</p>
            <div className="obcanvas-tools"><button onClick={() => void run(() => openSection('assets', selected.record!.id))}>打开项目资产库</button><button onClick={() => void run(() => openSection('storyboard', selected.record!.id))}>打开分镜工作区</button></div>
          </>}
        ...
        {panel === 'order' && <nav className="obcanvas-order"><p className="obcanvas-hint">{project?.editOrder !== undefined ? '当前场次按全片顺序显示，新镜头标为待排。' : '这里是当前场次内的顺序；跨场次成片顺序请在分镜工作区确认。'}</p>{shots.map((s, i) => <div data-order-shot={s.id} key={s.id}><button onClick={() => select(graph.nodes.find(n => n.record?.id === s.id)!, true)}>{project?.editOrder !== undefined ? project.editOrder.includes(s.id) ? '全片 ' + (project.editOrder.filter(id => fullCatalog.records.some(r => r.id === id && r.kind === 'shot')).indexOf(s.id) + 1) : '待排' : String(i + 1).padStart(2, '0')} · {s.title}</button><span data-shot-status={shotStatus(s)}>{shotStatus(s)}</span>{project?.editOrder === undefined && <><button aria-label={`上移镜头：${s.title}`} disabled={busy || i === 0} onClick={() => void run(() => records.moveShot(scene!, s.id, -1))}>上移</button><button aria-label={`下移镜头：${s.title}`} disabled={busy || i === shots.length - 1} onClick={() => void run(() => records.moveShot(scene!, s.id, 1))}>下移</button></>}</div>)}<button onClick={() => void run(() => openSection('storyboard', ''))}>去分镜工作区整理全片顺序</button></nav>}
      </aside>}
