...
export function ShotCard({ shot, position, scale, selected, disabled, onSelect, onMove }: { shot: FilmRecord; position: Position; scale: number; selected: boolean; disabled: boolean; onSelect: () => void; onMove: (p: Position) => void }) {
  ...
  return <button className={`obcanvas-node ${selected ? 'is-selected' : ''}`} data-node-id={shot.id} aria-label={`镜头卡：${shot.title}`} aria-pressed={selected} disabled={disabled}
    ...
    <span className="obcanvas-node-kicker">镜头记录</span><strong>{shot.title}</strong>
